            ...
            {navLinks.map((link) => (
              <div key={link.href} className="relative group">
                <Link
                  href={link.href}
                  className={cn(
                    "text-sm font-medium text-gray-700 hover:text-brand-700 transition-colors relative flex items-center gap-1",
                    link.className
                  )}
                >
                  {link.label}
                  ...
                  <div className="pl-4">
                    {link.children.map((child) => (
                      <Link
                        key={child.href}
                        href={child.href}
                        className="block px-4 py-2.5 text-sm text-gray-500 rounded-xl hover:bg-brand-50 hover:text-brand-700 transition-colors"
                        onClick={() => setMobileOpen(false)}
                      >
                        {child.label}
                      </Link>
